...
class LightningMage():
    ...
    def attack(self,SCREEN, target):
        ...
        if not self.attacking and not self.using:
            ...
            def check():
                if Hitbox((-30 if self.flip else +30 ,0),(self.width_factor,self.height_factor),self,1,1.5).detect_collision(SCREEN, target):
    ...
    def ability(self,SCREEN,target):
        if not self.using and not self.attacking:
            if self.stamina >= self.stamina_need:
                ...
                def check():
                    if Hitbox((0,0),(self.width_factor,self.height_factor),self,1,1.8).detect_collision(SCREEN, target):
